...
## Diff Precision — CRITICAL FOR SCORING

- **Match the reference style exactly.** The reference (cursor/gemini-3-flash) makes minimal changes and preserves all original structure. Do not "improve" formatting.
- **Preserve blank lines exactly.** Do not add extra blank lines between existing code lines. Do not remove blank lines unless the task explicitly requires it.
...
- **Minimal mutation.** Change only what is strictly required by the task. Do not reformat, reorganize, or "clean up" code.
- **Do not collapse lines.** When replacing content, maintain the original line structure. Do not join multiple lines into one.
...
- **When removing content:** If the reference removes a line, check whether it leaves a blank line in its place or collapses the space. Match that behavior exactly.
