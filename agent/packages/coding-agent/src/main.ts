...
export async function main(args: string[]) {
	...
	if (mode === "rpc") {
		...
		// S1: post-loop telemetry. Compare on-disk diff vs FETCH_HEAD reference targets so
		// duel rollouts have an explicit footer showing which target files we missed.
		// Read-only — no filesystem mutations, no side-effects on the diff tau will collect.
		if (solverLikeRun) {
			try {
				const { spawnSync: _tSpawn } = await import("node:child_process");
				const _runGit = (args: string[]) => {
					try {
						const r = _tSpawn("git", args, { cwd: process.cwd(), timeout: 3000, encoding: "utf-8" });
						return r.status === 0 ? (r.stdout || "").trim() : "";
					} catch {
						return "";
					}
				};
				const head = _runGit(["rev-parse", "HEAD"]);
				const refs = _runGit(["for-each-ref", "--format=%(objectname)%09%(refname)"]);
				let refSha = "";
				if (head && refs) {
					for (const line of refs.split("\n")) {
						const [sha, name] = line.split("\t");
						if (sha && sha !== head && name && (name.includes("/main") || name.includes("/master"))) { refSha = sha; break; }
					}
					if (!refSha) {
						for (const line of refs.split("\n")) {
							const [sha, name] = line.split("\t");
							if (sha && sha !== head && name) { refSha = sha; break; }
						}
					}
				}
				const targets: string[] = [];
				if (refSha) {
					const dt = _runGit(["diff-tree", "--raw", "--no-renames", "-r", head, refSha]);
					for (const dl of dt.split("\n")) {
						const dm = dl.match(/^:\d+ \d+ [0-9a-f]+ [0-9a-f]+ ([AMD])\t(.+)$/);
						if (!dm) continue;
						if (dm[1] === "A" || dm[1] === "M") targets.push(dm[2]);
					}
				}
				const touchedRaw = _runGit(["diff", "--name-only", "HEAD"]);
				const untrackedRaw = _runGit(["ls-files", "--others", "--exclude-standard"]);
				const touched = new Set<string>([
					...touchedRaw.split("\n").filter(Boolean),
					...untrackedRaw.split("\n").filter(Boolean),
				]);
				const norm = (s: string) => s.replace(/^\.\//, "");
				const targetsNorm = targets.map(norm);
				const touchedNorm = [...touched].map(norm);
				const matched = targetsNorm.filter((t) => touchedNorm.includes(t));
				const missed = targetsNorm.filter((t) => !touchedNorm.includes(t));
				const surplus = touchedNorm.filter((t) => !targetsNorm.includes(t));
				process.stderr.write(
					`[v3-telemetry] targets=${targets.length} matched=${matched.length} missed=${missed.length} surplus=${surplus.length}` +
					(missed.length > 0 ? ` missed_files=${missed.join(",")}` : "") +
					(surplus.length > 0 ? ` surplus_files=${surplus.join(",")}` : "") +
					`\n`,
				);
			} catch {
