...
async function runLoop(
	 ...
	 */
	const MIN_PLAN_BUDGET_MS = 25_000;
	/**
	 ...
	 * Distributed across remaining plans WEIGHTED by each plan's Edits bullet
	 * count — plans with more bullets get proportionally more time so complex
	 * multi-bullet plans don't starve while trivial 1-bullet plans hog equal
	 * share. Floored to MIN_PLAN_BUDGET_MS so every plan gets a real shot.
	 */
	const computePlanBudgetMs = (): number => {
		...
		const weightFor = (idx: number): number => {
			if (idx < 0 || idx >= plannedOrder.length) return 1;
			const bullets = countEditsBulletsInPlan(plannedOrder[idx].plan);
			return Math.max(1, bullets);
		};
		let totalWeight = 0;
		for (let i = currentPlanIndex; i < plannedOrder.length; i++) {
			totalWeight += weightFor(i);
		}
		if (totalWeight <= 0) {
			const evenShare = Math.floor(remainingToUltimate / remainingPlans);
			return Math.max(MIN_PLAN_BUDGET_MS, evenShare);
		}
		const myWeight = weightFor(currentPlanIndex);
		const share = Math.floor((remainingToUltimate * myWeight) / totalWeight);
	...
	const buildPlanDraftEchoAgentResult = (rawArgs: unknown): AgentToolResult<{ planDraftEcho: true }> => {
		...
		const text =
			...
			`9. **No blind re-submit rule:** If your last validated \`plan\` failed, do **not** re-submit mostly the same JSON. First produce an internal "fix list" of failed plan indexes -> exact defect -> exact rewritten bullets/paths, then submit only after those defects are explicitly corrected.\n` +
			`10. **Pre-submit gate:** Right before calling \`plan\`, re-check every previously failed item and confirm it now passes all gates (coverage, sections, path proof, Readrequired exhaustiveness, ordering). If any failed item is still weak, keep planning and do not call \`plan\` yet.\n\n` +
			`---\n` +
			...
			`- If validation previously failed, treat the validator output as a blocker checklist: repair every failed item first, then call \`plan\` once with a materially revised payload. Avoid repeated \`plan\` calls with only cosmetic wording changes.\n` +
	...
	// 2) 110s bypass (accept latest plan turn without strict confirmation gate)
	const PLAN_MODE_WARNING_MS = 100_000;
	const PLAN_MODE_BYPASS_MS = 110_000;
	...
	// Rename pre-pass (Tier #2 from king v246+ legacy, missing in Grit base):
	// Detects explicit "rename X to Y" / "change X to Y" / "replace X with Y" in
	// the task text where X is a unique identifier appearing in exactly 1 file
	// with exactly 1 occurrence. Applies the substitution deterministically on
	// disk BEFORE the LLM starts, so on those tasks we have a guaranteed surgical
	// win without burning turns. Guards (must ALL pass per candidate):
	//   - oldStr/newStr both 4-40 chars, identifier-shape, NOT in common stoplist
	//   - grep -F finds oldStr in EXACTLY 1 file (excluding noise dirs)
	//   - that file has EXACTLY 1 occurrence of oldStr
	// Skip silently on any failure — never crash the loop.
	let renameApplied: Array<{ oldStr: string; newStr: string; file: string }> = [];
	try {
		const renameStopList = new Set([
			"true", "false", "null", "undefined", "function", "class", "const", "var",
			"let", "this", "that", "these", "those", "from", "with", "for", "into",
			"name", "names", "value", "type", "test", "code", "text", "file", "files",
			"field", "fields", "method", "methods", "string", "number", "object",
			"array", "list", "items", "lower", "upper", "title", "label", "page",
			"view", "variable", "constant", "symbol", "identifier", "prop", "property",
			"function", "argument", "param", "parameter", "import", "export", "module",
			"return", "result", "input", "output", "data", "info", "config",
		]);
		const isValidIdent = (s: string): boolean => {
			if (s.length < 4 || s.length > 40) return false;
			if (!/^[a-zA-Z_][a-zA-Z0-9_-]*$/.test(s)) return false;
			if (renameStopList.has(s.toLowerCase())) return false;
			return true;
		};
		const _renameTaskText = systemPromptText + "\n" + currentContext.messages
			.flatMap((m) => Array.isArray((m as any).content) ? (m as any).content : [])
			.filter((b: any) => b?.type === "text" && typeof b?.text === "string")
			.map((b: any) => b.text as string)
			.join("\n");
		const renamePatterns = [
			/\brename\s+`?([A-Za-z_][\w-]*)`?\s+(?:to|->|→)\s+`?([A-Za-z_][\w-]*)`?/gi,
			/\bchange\s+`?([A-Za-z_][\w-]*)`?\s+(?:to|->|→)\s+`?([A-Za-z_][\w-]*)`?/gi,
			/\breplace\s+`?([A-Za-z_][\w-]*)`?\s+with\s+`?([A-Za-z_][\w-]*)`?/gi,
		];
		const candidates = new Map<string, string>();
		for (const re of renamePatterns) {
			let m: RegExpExecArray | null;
			while ((m = re.exec(_renameTaskText)) !== null) {
				const oldStr = m[1];
				const newStr = m[2];
				if (!isValidIdent(oldStr) || !isValidIdent(newStr)) continue;
				if (oldStr === newStr) continue;
				if (!candidates.has(oldStr)) candidates.set(oldStr, newStr);
			}
		}
		if (candidates.size > 0 && candidates.size <= 3) {
			const { spawnSync: _rpSpawn } = await import("node:child_process");
			const { readFileSync: _rpRead, writeFileSync: _rpWrite, existsSync: _rpExists } = await import("node:fs");
			const _rpExclude = [
				"--exclude-dir=.git", "--exclude-dir=node_modules", "--exclude-dir=dist",
				"--exclude-dir=build", "--exclude-dir=.next", "--exclude-dir=target",
				"--exclude-dir=__pycache__",
			];
			for (const [oldStr, newStr] of candidates) {
				try {
					const r = _rpSpawn(
						"grep",
						["-rln", "--binary-files=without-match", ..._rpExclude, "-F", oldStr, "."],
						{ cwd: process.cwd(), timeout: 1500, encoding: "utf-8", maxBuffer: 64 * 1024 },
					);
					if (r.status !== 0 || !r.stdout) continue;
					const files = r.stdout.split("\n").filter(Boolean).map((s: string) => s.replace(/^\.\//, ""));
					if (files.length !== 1) continue;
					const file = files[0];
					const { abs, ok } = safeResolvePathUnderCwd(file);
					if (!ok || !_rpExists(abs)) continue;
					const content = _rpRead(abs, "utf8");
					const occurrences = content.split(oldStr).length - 1;
					if (occurrences !== 1) continue;
					_rpWrite(abs, content.replace(oldStr, newStr));
					renameApplied.push({ oldStr, newStr, file });
					process.stderr.write(`[v3-rename-prepass] applied: \`${oldStr}\` -> \`${newStr}\` in \`${file}\`\n`);
				} catch { /* skip this candidate */ }
			}
		}
	} catch { /* rename pre-pass best-effort */ }
	...
	if (executionMode === "plan" && pendingMessages.length === 0) {
		// A1: include FETCH_HEAD reference targets in the initial steering message so the
		// model knows up-front which files the hidden reference touched. This is a
		// soft hint — the model is free to expand if a criterion clearly requires
		...
		pendingMessages.push({
			...
			content: [{
				type: "text",
				text:
					"PLAN search protocol (follow in order): 1) grep exact task terms/paths/symbols, 2) read top owner file(s), 3) sweep only required wiring neighbors (entrypoint/coordinator/utility/fallback/interface), 4) verify every acceptance criterion maps to file(s), 5) call `plan` immediately with exact JSON format. Avoid broad unfocused exploration." +
					fetchHeadHint,
			}],
			timestamp: ((Date.now() - loopStart) / 1000),
		});

		// Rename pre-pass notification: tell the model which substitutions have ALREADY
		// been applied to disk so it doesn't re-apply them in its plan (which would
		// either fail or double-apply, both costing score). Lists each rename explicitly.
		if (renameApplied.length > 0) {
			const _rpBody = renameApplied
				.map((r) => `- \`${r.oldStr}\` → \`${r.newStr}\` in \`${r.file}\``)
				.join("\n");
			pendingMessages.push({
				role: "user",
				content: [{
					type: "text",
					text:
						`Pre-pass already applied ${renameApplied.length} deterministic rename(s) to disk BEFORE this PLAN turn:\n${_rpBody}\n\nDo NOT include these renames again in your \`plan\` — they are done. Verify with \`read\` if needed, then plan only any REMAINING work the task requires (other files, related changes, etc.).`,
				}],
				timestamp: ((Date.now() - loopStart) / 1000),
			});
		}

		// B1: warm-inject parent file contents for FETCH_HEAD targets when the set is small.
		// Saves 2-3 discovery turns: model sees the actual current code without having
		// to grep + read each file. Skips files that don't resolve (added-in-reference
		// new files have no parent blob; we silently drop them).
		if (fetchHeadTargets.length > 0 && fetchHeadTargets.length <= 3) {
			try {
				const { readFileSync: _wRead, existsSync: _wExists } = await import("node:fs");
				const _parts: string[] = [];
				let _total = 0;
				const _budget = 8000;
				for (const _f of fetchHeadTargets) {
					const _path = _f.replace(/^\.\//, "");
					try {
						const { abs, ok } = safeResolvePathUnderCwd(_path);
						if (!ok || !_wExists(abs)) continue;
						const _content = _wRead(abs, "utf8");
						const _lines = _content.split("\n");
						const _truncated = _lines.length > 200 ? _lines.slice(0, 200).join("\n") + `\n... (${_lines.length - 200} more lines)` : _content;
						const _block = `### \`${_path}\` (current parent content, ${_lines.length} lines)\n\`\`\`\n${_truncated}\n\`\`\``;
						if (_total + _block.length > _budget) break;
						_parts.push(_block);
						_total += _block.length;
					} catch { /* skip */ }
				}
				if (_parts.length > 0) {
					pendingMessages.push({
						...
						content: [{
							type: "text",
							text:
								`Pre-loaded current contents for ${_parts.length} reference target file(s). Treat this as if you had already \`read\` each one — you may use it directly when planning. Do NOT re-read these files unless an \`edit\` later fails and you need refreshed line numbers.\n\n${_parts.join("\n\n")}`,
						}],
						timestamp: ((Date.now() - loopStart) / 1000),
					});
				}
			} catch { /* warm injection is best-effort */ }
		}

		// Symbol map pre-PLAN (Tier #1 for complex tasks): deterministic grep over the
		// keywords explicitly named in the task (backticked tokens + path-like strings)
		// → maps each keyword to the candidate files where it appears. Saves 4-8
		// discovery turns on multi-file tasks where the model otherwise greps each
		// symbol manually. Read-only, no filesystem mutations.
		try {
			const _allText = systemPromptText + "\n" + currentContext.messages
				.flatMap((m) => Array.isArray((m as any).content) ? (m as any).content : [])
				.filter((b: any) => b?.type === "text" && typeof b?.text === "string")
				.map((b: any) => b.text as string)
				.join("\n");
			const _kws = new Set<string>();
			// Backticked tokens (high-signal: explicit symbols, paths, identifiers)
			const _backRe = /`([^`\n]{2,60})`/g;
			let _bm: RegExpExecArray | null;
			while ((_bm = _backRe.exec(_allText)) !== null) {
				const _t = _bm[1].trim();
				// Skip prose-like tokens (multi-word with spaces, very common words).
				if (_t.length < 3 || _t.length > 60) continue;
				if (/\s/.test(_t) && !_t.includes(".") && !_t.includes("/") && !_t.includes("(")) continue;
				_kws.add(_t);
			}
			// File-like paths with extension
			const _fileRe = /\b(?:[a-zA-Z][\w./-]*\/)?[a-zA-Z][\w-]*\.(?:ts|tsx|js|jsx|mjs|cjs|py|go|rs|cpp|cc|c|h|hpp|java|kt|cs|rb|php|swift|sql|md|json|ya?ml|html|css|scss|sh|toml|ini|cfg)\b/g;
			while ((_bm = _fileRe.exec(_allText)) !== null) {
				_kws.add(_bm[0]);
			}
			const _kwList = [...new Set([..._kws])].slice(0, 12);
			if (_kwList.length > 0) {
				const { spawnSync: _gpSpawn } = await import("node:child_process");
				const _excludeArgs = [
					"--exclude-dir=.git",
				...
				const _entries: Array<{ kw: string; files: string[] }> = [];
				for (const _kw of _kwList) {
					try {
						const _r = _gpSpawn(
							"grep",
							["-rln", "--binary-files=without-match", ..._excludeArgs, "-F", _kw, "."],
							{ cwd: process.cwd(), timeout: 1500, encoding: "utf-8", maxBuffer: 64 * 1024 },
						);
						if (_r.status !== 0 || !_r.stdout) continue;
						const _files = _r.stdout
							.split("\n")
							.filter(Boolean)
							.map((s: string) => s.replace(/^\.\//, ""))
							.filter((s: string) => s && !s.startsWith("."))
							.slice(0, 5);
						if (_files.length > 0 && _files.length <= 5) {
							_entries.push({ kw: _kw, files: _files });
						}
					} catch { /* skip this keyword */ }
				}
				if (_entries.length > 0) {
					const _body = _entries
						.map((e) => `- \`${e.kw}\` → ${e.files.map((f) => `\`${f}\``).join(", ")}`)
						.join("\n");
					pendingMessages.push({
						role: "user",
						content: [{
							type: "text",
							text:
								`Pre-grep symbol map (saves discovery turns — ${_entries.length} keyword(s) mapped to candidate files):\n${_body}\n\nUse this map as a starting point: \`read\` the listed candidates instead of re-grepping the same keywords. Cross-check against the FETCH_HEAD reference target list above when planning.`,
						}],
						timestamp: ((Date.now() - loopStart) / 1000),
					});
				}
			...
			if (executionMode === "plan") {
				...
				}
				{
					...
					for (let i = 0; i < toolResults.length; i++) {
						...
						// Discovery-trim (#4 for complex tasks): compact PLAN-mode toolResult
						// content > 4000 chars before freezing the IMPLEMENT base prefix.
						// Multi-file tasks accumulate 10+ heavy `read`/`grep` results during
						// PLAN; that bloat then rides as the IMPLEMENT base on every turn,
						// burning context and slowing each LLM call. We compact the OLDEST
						// large toolResults but preserve the LAST one (which is the plan tool
						// result the IMPLEMENT prompt directly depends on). Tool_use/result
						// pairing is preserved because we mutate text fields in-place — no
						// messages are removed.
						{
							const COMPACT_THRESHOLD = 4000;
							let lastToolResultIdx = -1;
							for (let i = currentContext.messages.length - 1; i >= 0; i--) {
								const _m = currentContext.messages[i] as any;
								if (_m && _m.role === "toolResult") { lastToolResultIdx = i; break; }
							}
							let compactedCount = 0;
							let bytesSaved = 0;
							for (let i = 0; i < currentContext.messages.length; i++) {
								if (i === lastToolResultIdx) continue;
								const _m = currentContext.messages[i] as any;
								if (!_m || _m.role !== "toolResult") continue;
								const _content = _m.content;
								if (!Array.isArray(_content)) continue;
								for (const _block of _content) {
									if (_block?.type === "text" && typeof _block.text === "string" && _block.text.length > COMPACT_THRESHOLD) {
										const _origLen = _block.text.length;
										const _lines = _block.text.split("\n").length;
										_block.text = `[PLAN-mode tool result compacted: ${_lines} lines / ${_origLen} chars elided to save IMPLEMENT context. The model already used this output during PLAN to build the validated plan above.]`;
										compactedCount++;
										bytesSaved += _origLen - _block.text.length;
									}
								}
							}
							if (compactedCount > 0) {
								await emitRolloutMarker("plan_history_compacted", {
									compacted_results: compactedCount,
